...
def graficos():
    ...
    kwhs = [200, 180, 210]
    media_kwh = sum(kwhs) / len(kwhs)

    fig, ax = plt.subplots()
    ax.bar(meses, kwhs, color='skyblue')
    ax.axhline(media_kwh, color='red', linestyle='--', label=f'Média: {media_kwh:.1f} kWh')
    ax.set_ylabel("Consumo (kWh)")
    ax.set_title("Consumo Mensal de Energia")
    ax.legend()
    ...
    return render_template("graficos.html", imagem_base64=imagem_base64)
